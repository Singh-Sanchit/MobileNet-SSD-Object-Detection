...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--thr", default=0.4, help="confidence threshold to filter out weak detections")
    ...
    while True:
        ...
        blob = cv.dnn.blobFromImage(
            frame, inScaleFactor, (inWidth, inHeight), meanVal)
        net.setInput(blob)
        ...
        for i in range(detections.shape[2]):
            ...
            if confidence > args.thr:
                ...
                if classNames[class_id] == 'person':
                    ...
                    cv.putText(frame, "No of Person Detected - " + str(detections.shape[2]), (10,710),
                               cv.FONT_HERSHEY_DUPLEX, 0.5, (61,90,254))
        cv.imshow("CalmNet", frame)
        if cv.waitKey(1) >= 0:
            break
